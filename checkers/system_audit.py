...
def audit_cg_convergence(name: str) -> CheckReport:
    ...
    report.metrics["last_min_rc"] = round(cg.last_min_rc, 6) if cg.last_min_rc is not None else None
    report.metrics["converged"] = cg.converged
    report.metrics["lrmp_obj"] = round(cg.lrmp_obj, 2) if cg.lrmp_obj is not None else None
    report.metrics["lb"] = round(lb, 2)
    if not cg.converged:
        report.add_violation(
            "cg",
            "not_converged",
            f"达到 max_iterations 仍未将 LRMP 求到最优，last_min_rc={cg.last_min_rc}",
        )
    elif cg.last_min_rc is not None and cg.last_min_rc < -rc_tol:
        report.add_violation(
            "cg",
            "rc_tolerance",
            f"标记已收敛但 min_rc={cg.last_min_rc:.6f} < -{rc_tol}",
        )
    return report
